...
def DelayError(Exception):
    pass

def delay(func):
    def f(*args, **kwargs):
        if not args[0].ready: raise DelayedError()
        func(*args, **kwargs)
    return f
    ...
    def connect(self, connection_id):
        ...
        while not connected:
            ...
            while(not(res_lookup['success'])):
                res_lookup = self.request(res_lookup['next'], 'lookup', key=self.id)
            self.successor = res_lookup['next']
            if self.request(res_lookup['previous'], 'commit_successor', self.id, res_lookup['next']):
                connected = True
            else:
...
def main():
    Chord.m = 7
    r = reactor.Reactor()
    r.add_peer(peer_class = Chord, id = 'peer1', args = ("peer1",))
    r.add_peer(peer_class = Chord, id = 'peer2', args = ("peer2", "peer1"))
    r.run()
